package com.pulsequeue.metrics;
...
public class EventMetrics {
    ...
    public EventMetrics(MeterRegistry registry) {
        this.processedCounter = Counter.builder("pulsequeue.events.processed")
                .description("Events successfully processed and dispatched")
                .register(registry);
        this.failedCounter = Counter.builder("pulsequeue.events.failed")
                .description("Processing attempts that failed (before or without exhausting retries)")
                .register(registry);
        this.deadLetteredCounter = Counter.builder("pulsequeue.events.deadlettered")
                .description("Events that exhausted all retries and landed on the dead-letter queue")
                .register(registry);
        this.duplicateCounter = Counter.builder("pulsequeue.events.duplicate")
                .description("Deliveries skipped because the eventId was already processed")
                .register(registry);
        this.retriedCounter = Counter.builder("pulsequeue.events.retried")
                .description("Processing attempts that were retries (attempt 2+)")
                .register(registry);
        this.processingTimer = Timer.builder("pulsequeue.events.processing.duration")
